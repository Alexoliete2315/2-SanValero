...
public int getPrioridad(){
    ...
    int probabilidadTraer = 0;
    int probabilidadMensaje = 0;
    int probabilidadVigilar = 0;
    boolean prioridadesDistintas = true;
        ...
        while(true){
            try (ServerSocket server = new ServerSocket(puerto)) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    //cada una de las ordenes asignadas a los leguionarios
                    for (int j= 0; j < 5; j++) {
...
//                        dataOutputStream.writeUTF(ordenCifrada);
                        //saco la probabilidad de cada uno de los tipos
                        for (int k = 0; k < 3; k++) {
                            //hago un bucle para que las prioridades sean siempre distintas

                                probabilidadTraer = getPrioridad();
                                probabilidadMensaje = getPrioridad();
                                probabilidadVigilar = getPrioridad();

//                            System.out.println("P T: " + probabilidadTraer + "P M: " + probabilidadMensaje + "P V: " + probabilidadVigilar);
                        }
                        //se las mando a los decuriones
                        dataOutputStream.writeInt(probabilidadTraer);
                        dataOutputStream.writeInt(probabilidadMensaje);
                        dataOutputStream.writeInt(probabilidadVigilar);
                        System.out.println("Prioridades dadas");
                    }
                    ...
                    System.out.println("Ordenes dada a centurión.");
                }
            } catch (IOException e) {
